...
let NewTaskPage = () => {
  ...
  let catagoriesApiController = new CategoriesController();
  ...
  let fetchCategories = async () => {
    if(categories.length == 0){
      let categoriesData =  await catagoriesApiController.read();
      dispatch(categoreisActions.read(categoriesData));
    }
  }
  ...
  let save = async () => {
    ...
    let response = await tasksController.save(task);
    alert(response.message);
    if (response.status) {
      dispatch(tasksActoins.create(response.object));
      clear();
    }
  };
